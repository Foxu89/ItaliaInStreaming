...
class AnimeUnity(
    private val showLogo: Boolean = false  // 🔧 Default: logo disattivati
) : MainAPI() {
    ...
    // 🔧 FUNZIONE PER OTTENERE LOGO TMDB (SPECIALE PER ANIME)
    private suspend fun fetchTmdbLogoUrl(
        ...
        return try {
            val url = "$tmdbAPI/tv/$tmdbId/images"
            ...
            // 🔧 LOGICA SPECIALE PER ANIME: preferisci EN/JA invece di IT
            val preferredLang = when (appLangCode?.substringBefore("-")?.lowercase()) {
                "ja", "jp" -> "ja"  // Giapponese per utenti JA
                else -> "en"         // Inglese per tutti gli altri (incluso IT)
            }
            
            // Cerca prima nella lingua preferita
            var svgFallback: String? = null
            for (i in 0 until logos.length()) {
                val logo = logos.getJSONObject(i)
                if (logo.optString("iso_639_1") == preferredLang) {
                    if (isSvg(i)) {
            ...
            // Se non trova nella lingua preferita, cerca in inglese
            ...
            // Se non trova inglese, cerca in giapponese
            var jaSvgFallback: String? = null
            for (i in 0 until logos.length()) {
                val logo = logos.getJSONObject(i)
                if (logo.optString("iso_639_1") == "ja") {
                    if (isSvg(i)) {
                        if (jaSvgFallback == null) jaSvgFallback = logoUrlAt(i)
                    } else {
                        return logoUrlAt(i)
                    }
                }
            }
            if (jaSvgFallback != null) return jaSvgFallback
            
            // Prendi il primo non-SVG, o il primo in assoluto
            for (i in 0 until logos.length()) {
                if (!isSvg(i)) {
                    return logoUrlAt(i)
                }
    ...
    private suspend fun searchTmdbAnime(title: String, malId: Int?): Int? {
        return try {
            ...
            if (results != null && results.length() > 0) {
                // Prendi il primo risultato (di solito il più rilevante)
                val firstResult = results.getJSONObject(0)
                firstResult.optInt("id", 0).takeIf { it > 0 }
            } else {
                null
            }
        } catch (e: Exception) {
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        // 🔧 CERCA TMDB ID E LOGO (SOLO SE showLogo = true)
        val logoUrl = if (showLogo && anime.anilistId != null) {
            val tmdbId = getTmdbIdFromAnilist(anime.anilistId)
            if (tmdbId != null) {
                fetchTmdbLogoUrl(tmdbId, lang)
            } else {
                null
            }
        } else null
